...
// updated by DevHelper_Helper_ShippableHelper at 2016-05-11T04:10:18+00:00

/**
 * Class bdAd_ShippableHelper_UpdaterCore
 * @version 1
...
class bdAd_ShippableHelper_UpdaterCore
{
    ...
    {
        if (isset($GLOBALS[bdAd_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']])) {
            if (XenForo_Application::debugMode()) {
                die(sprintf('Updater for %s has been setup by %s already!', $config['apiUrl'],
                    $GLOBALS[bdAd_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']]));
            }
            return;
        }
        $GLOBALS[bdAd_ShippableHelper_Updater::KEY]['setupMethod'][$config['apiUrl']] = __METHOD__;
...
EOF;
        ...
        if (!empty($_GET[self::PARAM_DOWNLOAD])
            ...
            if (!empty($data[$addOnId]['links']['content'])) {
                $tempFile = bdAd_ShippableHelper_TempFile::download($data[$addOnId]['links']['content']);
    ...
    private static function _updateSelf(array $config)
    {
        ...
        $configOptionId = bdAd_ShippableHelper_Updater::getConfigOptionId($config['apiUrl']);
        ...
        if ($configVersion < bdAd_ShippableHelper_Updater::$_version) {
            ...
            $config['version'] = bdAd_ShippableHelper_Updater::$_version;
            $configUpdated[] = '$config["version"]';
        }

        $addOnIdsUpdated = false;
        foreach ($GLOBALS[bdAd_ShippableHelper_Updater::KEY][$config['apiUrl']]['addOns'] as $addOnId => $enabled) {
        ...
        if (count($configUpdated) > 0) {
            ...
            if (XenForo_Application::debugMode()) {
                XenForo_Helper_File::log(__CLASS__, sprintf('%s($apiUrl=%s): $configVersion=%d,'
                    ...
                    var_export(bdAd_ShippableHelper_Updater::$_version, true),
    ...
    {
        $configOptionId = bdAd_ShippableHelper_Updater::getConfigOptionId($apiUrl);
        $templateTitle = $configOptionId;
        $templateModKey = $configOptionId;
    ...
    private static function _refreshData($apiUrl, array $addOnIds, $forceRefresh)
    {
        ...
        if (empty($data)
            || $data['version'] < bdAd_ShippableHelper_Updater::$_version
            || $data['timestamp'] < XenForo_Application::$time - 86400
        ) {
            $data = array(
                'version' => bdAd_ShippableHelper_Updater::$_version,
                'timestamp' => time(),
    ...
    private static function _fetchData($url, array $addOnIds, $accessToken = '')
    {
        ...
        $url .= sprintf('%s_version=%d', ((strpos($url, '?') === false) ? '?' : '&'),
            bdAd_ShippableHelper_Updater::$_version);
    ...
    private static function _getCache($apiUrl)
    {
        ...
        $data = $dataRegistryModel->get(bdAd_ShippableHelper_Updater::KEY);
    ...
    private static function _setCache($apiUrl, array $thisData)
    {
        ...
        $dataRegistryModel->set(bdAd_ShippableHelper_Updater::KEY, $data);
        return true;
    }
